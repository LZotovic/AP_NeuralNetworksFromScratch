...
def sgd_update(x_b: np.ndarray, y_b: np.ndarray, w: np.ndarray, eta: float) -> np.ndarray:
    ...
    for j in range(len(w)):
        gradient[j] = np.mean((y_pred - y_b) * (x_b ** j))
...
def train_minibatch(x: np.ndarray, y: np.ndarray, degree: int, eta: float, batch_size: int, n_epochs: int, seed: int = 0):
    ...
    """
    rng = np.random.default_rng(seed)
    w = rng.normal(0, 0.1, degree + 1)

    losses = []

    for epoch in range(n_epochs):
        permute = rng.permutation(len(x))
        x_shuffle = x[permute]
        y_shuffle = y[permute]
...
def create_gif(x: np.ndarray, y: np.ndarray, snapshots: list[np.ndarray], snapshot_every: int, path: str = "SGD_training_animation.gif"):
    ...
    anim = animation.FuncAnimation(fig, update, frames=len(
        snapshots), init_func=init, blit=True)
...
def main():
    ...
    eta_fb = 7e-3  # this one will likely be too high, adjust as needed
    ...
    # these may be too extreme, adjust as needed
    etas = [2e-4, 1e-3, 3e-3, 1e-2, 4e-2]
    plt.figure()
    for eta in etas:
        _, losses = train_minibatch(
            x, y, degree, eta=eta, batch_size=16, n_epochs=2000)
    ...
    bs_list = [1, 4, 16, 64, N]
    eta_fixed = 1e-2  # this one will likely be too high, adjust as needed
    plt.figure()
    for b in bs_list:
        _, losses = train_minibatch(
            x, y, degree, eta=eta_fixed, batch_size=b, n_epochs=2000)
    ...
    eta_anim, b_anim = 2e-2, 16
    epochs_anim, snap_every = 2000, 1
    _, _, snapshots = train_minibatch_snapshots(
        x, y, degree, eta=eta_anim, batch_size=b_anim, n_epochs=epochs_anim, snapshot_every=snap_every)
    create_gif(x, y, snapshots, snapshot_every=snap_every,
               path="SGD_training_animation.gif")
