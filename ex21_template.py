...
class Linear():
    ...
    def backward(self, grad_output):
        ...
        self.grad_weight = self.input.T @ grad_output / self.input.shape[0]
...
def Softmax(input):
    ...
    output = np.exp(input - np.max(input, axis=1, keepdims=True)) / np.sum(
        np.exp(input - np.max(input, axis=1, keepdims=True)), axis=1, keepdims=True)
    return output
